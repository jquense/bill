...
function match(selector, tree) {
  ...
  var matches = findAll(tree, compiled, undefined, true);
  return matches;
}
...
function findAll(root, test, getParent, includeSelf) {
  ...
  if (includeSelf && _react2['default'].isValidElement(root) && test(root, getParent)) {
    found.push(root);
  }
  ...
  _react2['default'].Children.forEach(children, function (child) {
    ...
    if (_react2['default'].isValidElement(child) && test(child, parent)) {
      found.push(child);
    }
